...
export function computeTransform(venue, width, height, insets = {}) {
...
function nodeShape(ctx, p, type, r) {
  ctx.beginPath();
  switch (type) {
    case "exit":
...
function crosshatch(ctx, p, r, color) {
  ctx.save();
...
export function renderFrame(ctx, opts) {
  const {
    venue, step, width, height, transform: t, hoverId,
    focusedElementId, focusedPath, tick,
  } = opts;
  ...
  const nodeById = {};
  venue.nodes.forEach((n) => (nodeById[n.id] = n));
  const nodeState = {};
  const edgeState = {};
  if (step) {
    step.nodes.forEach((s) => (nodeState[s.id] = s));
    step.edges.forEach((s) => (edgeState[s.id] = s));
  }
  ...
  venue.nodes.forEach((n) => {
    ...
    const baseR = n.type === "exit" || n.type === "seating" || n.type === "platform" ? 13 : 10;
    ...
    grad.addColorStop(0, `rgba(${r},${g},${bl},${alpha * (status === "critical" ? pulse : 1)})`);
    ...
    ctx.textAlign = "center";
    const above = n.type === "gate" || n.type === "concession" || n.type === "restroom";
    ctx.fillText(
      n.name,
      p.x,
      above ? p.y - radius - (status === "critical" ? 22 : 12) : p.y + radius + 16
    );
    ctx.restore();
  });
}

export function hitTest(venue, t, mx, my) {
  let best = null;
  venue.nodes.forEach((n) => {
    const p = project(n, t);
    const d = Math.hypot(p.x - mx, p.y - my);
    if (d < 22 && (!best || d < best.d)) best = { node: n, d };
  });
