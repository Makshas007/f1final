...
const severityColor = (s: StepResult): string => {
  if (
    s.nodes.some((n) => n.status === "critical") ||
    s.edges.some((e) => e.status === "critical")
  )
    return "#EF4444";
  if (
    s.nodes.some((n) => n.status === "warning") ||
    s.edges.some((e) => e.status === "warning")
  )
    return "#F97316";
  if (s.bottlenecks.length) return "#FBBF24";
  return "#10B981";
};
